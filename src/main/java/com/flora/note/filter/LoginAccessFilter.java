 ...
 * 拦截所有的资源/*
 * 需要被放行的资源：
 * 1、指定页面，放行（用户无需登录的即可访问的页面：例如：登录页面login.jsp、注册页面register.jsp)
 * 2、静态页面，放行（存放在statics目录下的资源，例如：js/css/images等）
 * 3、指定行为，放行（用户无需登录即可执行的操作，例如：登录操作actionName=login等）
 * 4、登录状态，放行（判断session作用域中是否存在user对象，存在则放行，不存在则拦截）
 * <p>
 * 免登录（自动登录）
 * 通过cookie对象实现
 * 什么时候使用免登录：
 * 当用户处于未登录的状态，且去请求需要登录才能访问的资源时，调用自动登录功能
 * 目的：让用户处于登录状态（自动调用登录方法）
 * 实现：从cookie对象中获取用户的姓名和密码，自动执行登录操作
 * 1、获取cookie数组 request.getCookie()
 * 2、判断cookie数组
